...
\end{figure}
	...
	\hspace{1cm}ก่อนจะลบบทบรรยายนั้น จะเป็นต้องหาบทบรรยายในภาพให้ได้เสียก่อน โดยบทบรรยายของอนิเมะนั้น มักจะขึ้นบริเวณด้านล่างของหน้าจอ และนอกจากนี้ บทบรรยายอนิเมะมักจะใช้ขอบของตัวอักษรเป็นสีดำอีกด้วย ด้วยสมบัตินี้เองทำให้จึงสามารถหาบริเวณบนเฟรมที่เป็นบทบรรยายได้โดยจะมีวิธีหาพื้นที่ซึ่งเป็นบทบรรยายดังนี้

	\clearpage
	
	\begin{algorithm}[H]
		\caption{Subtitle Finding Tecnique}
		\KwIn{
			\\
		...
		\SetKwProg{Fn}{Function}{:}{}
		\Fn{\FMain{$u$}}{
			$ D =  $ thresholding color in subtitle's edge color range of $u$ \\
			$ D =  $ inverse $D$ to select region inside subtitle edge \\
			$ D = $ remove region that attach to edge of image $D$ \\
			$ D = $ remove region that too big to be subtitle from $D$ \\
			$ D = $ remove region that too small to be subtitle from $D$ \\
			$ D = $ dilate with subtitle's edge width\\
			\textbf{return} D
		}	
	\end{algorithm}
	...
	\begin{table}[H]
		...
		\begin{tabular}[ht]{|l|c|c|c|c|}
			...
			\hline
			ไทย & 23,222,220 & 24083,125 & 2,141,201 & 9.22 \\
	...
	\begin{algorithm}[H]
		...
		\SetKwFunction{FMain}{BorrowFrameInpaint}
		\SetKwProg{Fn}{Function}{:}{}
		\Fn{\FMain{$u, D, v$}}{
			...
			\textbf{return} $MultiSplitBergmanColorInpaint(u,D,\lambda, \theta, g, N_0, N_1, N_2, \varepsilon,1,m)$
		}	
	\end{algorithm}
	\clearpage
	...
	\begin{algorithm}[H]
		...
		\KwOut{inpainted frame}    
		\SetKwFunction{FMain}{SkipFrameInpaint}
		\SetKwProg{Fn}{Function}{:}{}
		\Fn{\FMain{$u, D, v$}}{
			$U = CopyByDomain(u,D,\vec{0})$ \\
			$V = CopyByDomain(v,D,\vec{0})$ \\
	...
	\begin{algorithm}[H]
		...
		\KwOut{inpainted frame}    
		\SetKwFunction{FMain}{BorrowSkipFrameInpaint}
		\SetKwProg{Fn}{Function}{:}{}
		\Fn{\FMain{$u, D, v$}}{
			$U = CopyByDomain(u,D,\vec{0})$ \\
			$V = CopyByDomain(v,D,\vec{0})$ \\
	...
	\begin{table}[H]
		...
		\begin{tabular}[ht]{|l|c|c|c|c|}
			...
			ยืมเฟรม & 132.7787761 & 32.1969618 & 0.9654718\\
			ข้ามเฟรม & 89.29141836 & 29.0667072 & 0.9407602 \\
			ยืมเฟรมและข้ามเฟรม & 75.75894242 & 29.326696 & 0.9453868 \\
			\hline
		\end{tabular}
...
\begin{table}[H]
	...
	\begin{tabular}[ht]{|l|c|c|c|c|}
		...
		\hline
		สปริทเบรกแมน & * & * & * \\
		ที่คิดค้นขึ้น & 75.75894242 & 29.326696 & 0.9453868 \\
		\hline
	\end{tabular}
