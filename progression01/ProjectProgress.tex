...
\end{table}
	...
	\begin{algorithm}[H]
		...
		\Fn{\FMain{$u, \lambda, \theta, N_{gs}, N_0,N_1,N_2, \varepsilon,c,m$}}{
			...
			\If{c < m}{
				...
				$u = CopyResultByLambda(u,\lambda,R)$
			}
			\uIf{$c = 1$}{$N=N_0$}
			\uElseIf{$c = m$}{$N = N_2$}
			\Else{
	...
	\\ 1. ขั้นตอนสำหรับการคัดลอกผลลัพธ์ที่ได้จากการทำพีระมิดรูปภาพจากระดับชั้นที่ภาพมีขนาดเล็ก มาสู่บริเวณที่เป็นโดเมนต่อเติมของภาพในระดับชั้นขนาดใหญ่
	\begin{algorithm}[H]
		\caption{Copy image inside inpaint domain by $\lambda$ }
		\KwIn{
			\\
			\hspace{1cm} $u$ is image which is damaged image \\
			\hspace{1cm} $\lambda$ regulazation paramter lambda from equation (\ref{e2})\\
			\hspace{1cm} $v$ is image which will copy to u image
		}
		\KwOut{image}    
		\SetKwFunction{FMain}{CopyResultByLambda}
		\SetKwProg{Fn}{Function}{:}{}
		\Fn{\FMain{$u, \lambda, v$}}{
			\textbf{Initialize} $height = $ height of $u$, $width = $ width of u \\
			\For{i = 0; i < height; i++}{
				\For{j = 0; j<i; j++}{
					\If{$\lambda_{i,j} = 0$}{$u_{i,j} = v_{i,j}$}
				}		
			}
			\textbf{return} $ u $ 
		}	
